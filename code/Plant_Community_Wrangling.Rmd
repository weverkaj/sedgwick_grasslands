...
f = list.files(path = here("data/2023/plant_community/surveys/"))%>%
  as_tibble()%>%
  mutate(data = map(value, ~read_csv(paste0(here("data/2023/plant_community/surveys/"), .x), col_types = cols(quad = col_character()))))%>%
  unnest(data)
```
...
```
```{r}
soil_sample_data <- average_N_and_species_summary_data %>%
  filter(quad == "ss")%>%
  mutate(treatment = tolower(treatment),
    plot= as.character(plot))%>%
...
```{r}
ggplot(soil_sample_data, aes(x=root_mass_cor, y=avg_sla, color = species)) +  geom_point()
```
```{r}
ggplot(soil_sample_data, aes(x=root_mass_cor, y=species_avg_N, color = species)) +  geom_point()
```
