...
leaf_CN_formatted = leaf_CN %>%
  clean_names()%>%
   rename(sample_ID = bypass, mass= x7) %>%
   rename(leaf_nitrogen= x0_000_12, leaf_carbon = x0_000_13)%>%
...
leaf_CN_formatted <- leaf_CN_formatted %>%
  mutate(CNratio=leaf_carbon/leaf_nitrogen, 2)
  
```

```{r}
ggplot(leaf_CN_formatted, aes(x=species, y=CNratio)) +  geom_point()
>>>>>>> ec64eaae43f5fe3c3412b4f01ba971783b0fa870
```
